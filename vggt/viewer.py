...
Usage: python viewer.py examples/IMG_4708.glb
"""
...
def load_glb(path: str) -> tuple[np.ndarray, np.ndarray, list[trimesh.Trimesh]]:
    ...
    for name, geom in scene.geometry.items():
        # Apply scene graph transform (returns (matrix, geom_name))
...
app = FastAPI()
VISER_PORT = 8081  # will be set from args
...
async def chat_ws(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            if not msg:
                continue
            # Placeholder response — swap in LLM call here
...
def main():
    ...
    # --- Start viser in a background thread ---
